...
> **ווינדוס חוסם סקריפטים כברירת מחדל.** אם תקבלו
> `running scripts is disabled on this system` — לא עשיתם שום דבר לא נכון,
> זו הגדרת אבטחה של המערכת. לכל סקריפט יש גרסת `.cmd` שעוקפת אותה בבטחה
> (רק בשביל הסקריפט הזה, בלי לשנות הגדרה כלל־מערכתית): במקום
> `.\scripts\setup.ps1` להריץ `.\scripts\setup.cmd`. אפשר גם פשוט
> ללחוץ עליהם לחיצה כפולה מתוך סייר הקבצים — `start-server.cmd` עולה גם בלי
> שום פרמטר: הוא קורא כברירת מחדל מתיקיות Videos ו‑Downloads, ותמיד אפשר
> לגרור קובץ ישירות לתוך הדף.
...
**בווינדוס — הכי פשוט, בלי להקליד נתיבים:**
...
# מתי הדלת נפתחה: מסמנים פעם אחת את המלבן שבו הדלת נמצאת (שברים מהתמונה),
# ומאז שואלים עליו כמה פעמים שרוצים — בלי לפענח את הווידאו מחדש
.venv/bin/vscan zone add --video cam1 --name "דלת כניסה" --box 0.42,0.30,0.14,0.38
.venv/bin/vscan zone scan --name "דלת כניסה" --report door.html

# מי נכנס ומי יצא: מותחים קו במקום שבו עוברים, והמערכת סופרת חציות עם כיוון
.venv/bin/vscan line add --video cam1 --name "שער" --line 0.5,0.1,0.5,0.9
.venv/bin/vscan line scan --name "שער" --direction out --report left.html
```
...
**מי נכנס ומי יצא** — באותה לשונית **אזורים**, לבחור "קו חצייה" במקום "אזור":
מותחים קו על הפתח, החץ הירוק מראה איזה כיוון נחשב "נכנס" (כפתור "החלף כיוון"
הופך אותו), נותנים שם ושומרים. ואז בחיפוש: "מתי מישהו יצא מהשער".

**דלת, שער, קופה, חניה** — לשונית **אזורים**: בוחרים סרטון, גוררים עם העכבר
מלבן על התמונה, נותנים לו שם ושומרים. מאז אפשר ללחוץ **חיפוש באזור**, או פשוט
להקליד את שם האזור בתיבת החיפוש ("מתי הדלת נפתחת") — המערכת תזהה שמדובר באזור
ותריץ סריקה מקומית על התמונות הממוזערות. גם על הקלטה של 12 שעות זה לוקח שניות
ולא עולה כלום.
