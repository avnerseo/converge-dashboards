...
SYSTEM_PROMPT = (
    ...
    "The frames are consecutive samples from the same camera, in time order, a "
    "second or two apart. Judge each numbered frame on what is visible in it - "
    "but when the operator asks about a change (something appearing, leaving, "
    "moving, being taken or left behind), compare a frame with the ones before "
    "it and report the frame where the change is first visible.\n"
    "Report a frame only when the described thing is actually shown. If you are "
    "unsure, give it a low confidence rather than leaving it out - the operator "
    "reviews the hits, so a hedged hit is more useful than a silent miss, but "
    "an invented one is worse than both.\n"
